...
**État : phases 0 → 6 terminées ; Build plan v2 en cours — Phases 7 (pivot expédition,
`schemaVersion 4`), 8 (bestiaire & biomes) et 9 (scène de combat spatiale) terminées,
Phase 10 (animations & planification simultanée) à venir.** 129 tests verts, déployé sur
...
## Phase 8 — Bestiaire & biomes
- **Bestiaire : 17 ennemis** (14 + un boss PAR biome), `ogre_boss` supprimé. Forêt : gelée, gobelin,
  loup, araignée (poseur de poison), dryade (soigneuse), **Tréant ancestral** (boss). Château :
  squelette, archer squelette (chargeur télégraphié), chevalier noir, nécromancien (invocateur),
  **Roi des ossements** (boss). Volcan : diablotin (poseur de brûlure), gelée de magma, cultiste
  (invoque désormais un diablotin), chamane, golem (chargeur), **Seigneur des braises** (boss).
  **Le boss final varie par run** : c'est celui du biome du dernier segment.
- **Invocations intra-biome** : tout `summons` d'un ennemi appartient au bestiaire de son biome
  (nécromancien → squelette, cultiste → diablotin) — testé.
- **Tables de rencontres par palier** : `BiomeEncounters.combat` devient `string[][][]`
  (paliers faible → fort) ; palier choisi par la progression p (`tierFor`, exporté de stateMachine),
  rotation déterministe sur l'index du nœud À L'INTÉRIEUR du palier. Élites/boss : tables simples.
  Chaque biome expose son `bestiary: string[]` ; test de pureté : toute vague générée ⊆ bestiaire.
- **Sprites** : 13 nouvelles grilles programmatiques (format Phase 5 inchangé), boss en 16×14-16.
  Garde ajoutée (`pixelData.test.ts`) : grille rectangulaire + toute clé non transparente a une
  couleur dans sa palette. Galerie vérifiée visuellement via Playwright (rendu OK).
  L'échelle boss de l'UI lit `ENEMIES[type].isBoss` (plus de type codé en dur).
- **Sweep d'équilibrage** (bot naïf strike-only, soin au repos, 20 seeds × bandes courte/moyenne/longue
  × équipes 1/2/4). Réglages retenus : `enemyHpPerPlayer` 1 → **0.85** ; pacing 60/35 → **35/20** ;
  `restHealPct` 40 → **50** ; élite 1.15/1.05 → **1.1/1.0** ; dégâts du bestiaire ≈ −20 % ; vagues
  d'élite plafonnées à 3 ennemis. Résultats finaux : courte n=4 : boss atteint 8/20, progression
  moyenne 78 % ; moyenne n=4 : 2/20, 67 % ; n=2 courte : 48 % ; n=1 : ~25-29 % (survit largement au
  premier nœud, décision Phase 1 tenue). **Victoires 0/20 partout : attendu et assumé** — le kit de
  base (frappe + provocation) n'a ni soin ni burst ; la courbe gagnable arrive avec l'assemblage de
  kits en Phase 11 (c'est le pivot « stratégie front-loadée »). Les petites équipes (1-2) restent
  fragiles : à retravailler en Phase 11 (courbe par taille d'équipe).
- Tests : 125 verts (15 fichiers) dont `phase8.test.ts` (pureté des biomes, paliers, chargeur
  télégraphié, applicateurs poison/brûlure, plafond d'invocation) ; les tests d'équilibrage sont
  désormais relatifs à `BALANCE` (plus de valeurs codées en dur). `npm run build` vert.

## Phase 9 — Scène de combat spatiale & décors de biome
- **Modèle de rendu du combat** : UN canvas de scène **persistant** (320×180 logique, étiré en CSS
  `aspect-ratio 16/9`, `getScene` dans `ui/scene/scene.ts`) conservé dans `UiState.scene` à travers
  les re-renders (testé : même élément avant/après) — c'est la fondation de la boucle rAF de la
  Phase 10. Redessiné à chaque changement d'état pour l'instant. Invariant C2 tenu : la scène LIT
  l'état, n'écrit jamais dedans ; positions calculées à la volée (`ui/scene/layout.ts`), jamais stockées.
- **Overlays DOM = « plaques »** positionnées en % au-dessus du canvas : elles portent TOUT le contrat
  `data-*` (`data-enemy`, `data-player`, `data-target`, `data-intent`, `data-build`) et le ciblage au
  clic (plaque = `<button>` quand ciblable). jsdom sans canvas : le canvas expose `data-scene-biome`
  en marqueur inerte, toute la logique testée passe par les plaques.
- **Mise en scène JRPG** : équipe à gauche, ennemis à droite, **3 rangs maximum** puis débord en
  colonnes (équipe : 1 col ≤ 3, 2 ≤ 6, 3 ≤ 8 ; ennemis : 2 cols × 3 = plafond moteur 6) — en dessous
  de 3 rangs les plaques ne masquent jamais le sprite du rang suivant (leçon des captures Playwright).
  Boss dessiné à l'échelle 3, joueurs/ennemis à 2. Sprites des morts retirés de la scène (la plaque
  💀 reste) ; joueurs à terre estompés (alpha 0.35).
- **Plaques compactes** : nom + PV + statuts + kit (une ligne, ellipse) ; énergie/menace reléguées en
  infobulle (l'énergie est toujours pleine au moment de planifier). `data-sprite` (signature
  d'apparence) porté par la plaque — le test de fidélité customisation↔jeu passe par là désormais.
- **Décors programmatiques par biome** (D4, `ui/scene/backdrop.ts`) : dégradé de ciel + astre +
  silhouettes (rangées d'arbres en dents de scie / muraille crénelée + torches / cône de volcan à
  cratère luisant + fissures de lave) + sol strié. Variations en dents de scie = fonctions
  déterministes de x (décor stable image par image, pas de PRNG).
- **`sprite.ts` refactorisé en primitives partagées** (`drawMapAt`, `charLayers`, `drawCharAt`,
  `drawEnemyAt`, `spriteSignature`) ; correctif au passage : une apparence fraîchement jointe vaut
  `'default'` (pas une couleur CSS) → fallback de teintes lisibles au lieu d'un personnage tout noir
  (défaut présent depuis la Phase 5, criant sur la scène).
- **Seed locale rejouable** : le mode « Jouer en local » lit `?seed=` dans l'URL (même parsing que le
  champ en ligne) — utile pour rejouer/partager et pour les vérifications visuelles.
- Acceptation vérifiée : combat joué de bout en bout dans la scène (tests jsdom via plaques) ;
  captures Playwright à 1/4/8 joueurs sur les trois biomes (volcan/château/forêt) — lisible partout ;
  le décor suit `data-scene-biome` = biome du nœud courant (testé). 129 tests verts, build vert.
